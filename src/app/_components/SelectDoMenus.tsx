  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 w-full py-4 px-4">
      {(['all', ...(DO_MAP.keys())]).map((region) => {
        return (
          <Link key={region} href={`/map-page/${region === 'all' ? 'all' : DO_MAP.get(region as Do)}`} className="w-full hover:bg-gray-100 rounded-lg p-5 bg-gray-50 text-center font-bold">
